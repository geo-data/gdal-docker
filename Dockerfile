...
RUN apt-get update -y \
    && apt-get install -y --fix-missing --no-install-recommends \
       ...
       python3-dev python3-numpy python3-lxml \
...
ENV LD_LIBRARY_PATH=/usr/local/lib64/:/usr/lib/:/opt/FileGDB_API-64gcc51/lib
RUN ldconfig
...
RUN if test "${GDAL_VERSION}" = "master"; then \
    ...
    --with-fgdb=/opt/FileGDB_API-64gcc51 \
    --with-hide-internal-symbols \
    --with-jpeg12 \
    --with-python=/usr/bin/python3 --with-poppler --with-spatialite --with-mysql --with-liblzma \
...
ENV LD_LIBRARY_PATH=/usr/local/lib:/usr/local/lib64:/usr/lib/:/opt/FileGDB_API-64gcc51/lib:${LD_LIBRARY_PATH}
